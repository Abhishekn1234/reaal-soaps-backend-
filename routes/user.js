...
router.get('/', async (req, res) => {
    try {
      ...
      res.json({ mobileNumber: user.mobileNumber, username: user.username });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Server error' });
    }
...
router.put('/edit-details', async (req, res) => {
    try {
      const { mobileNumber, username, newMobileNumber } = req.body; // Assume new mobile number might be updated
      ...
      user.mobileNumber = newMobileNumber || mobileNumber; // Update with new mobile number if provided
